...
1. **Event Reception**: An upstream system sends webhook events to your application
2. **Event Processing**: The application validates, normalizes, and enriches the event
...
5. **Consumer Processing**: Workers consume and process events from Redis
6. **Response Handling**: Results are dispatched back to downstream systems
...
sequenceDiagram
    ...
    Source->>App: Webhook Event
    App->>App: Validate & Normalize Event
    App->>Redis: XADD (publish to stream)
    Redis->>Redis: Store in Stream
    Worker->>Redis: XREAD/XREADGROUP (consume)
    Redis->>Worker: Event Data
...
```toml
[project.entry-points."abstract_backend.backends.queue.service"]
redis = "abe_plugin.backends.queue.service.redis:RedisMessageQueueBackend"
```

When you set `QUEUE_BACKEND=redis` in your environment, the host application automatically discovers and loads the Redis backend.
